...
FE_VulkanAllocatedBuffer* VulkanCreateIndexBuffer(FE_VulkanInfo* vkInfo, Uint32 indexCount, SizeT* outIndexBufferId)
{
	...
	FE_VulkanBufferCount bufferCount = {
		.buffer = indexBuffer->buffer,
		.currentCount = 0,
	};

	FE_ListPush(vkInfo->indexBuffers, bufferCount);
...
void VulkanAddDrawIndexedIndirectCommandsBuffer(FE_VulkanInfo* vkInfo, Uint32 indexCount, SizeT indexBufferId)
{
	//must be same amount of descriptor ans drawIndexedIndirectCmd
	vkInfo->indexBuffers.data[indexBufferId].currentCount = indexCount;
...
void VulkanBindBuffers(FE_VulkanInfo* vkInfo, VkCommandBuffer cmdBuffer, SizeT indexBufferId, SizeT vertexBufferId)
{
	...
	vkCmdBindIndexBuffer(vkInfo->cmdBuffers[vkInfo->currentFrame], vkInfo->indexBuffers.data[indexBufferId].buffer, 0, VK_INDEX_TYPE_UINT32);
