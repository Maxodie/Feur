 ...
 */
  ...
  * ===============================================================
  */
#ifndef FE_NK_GLFW_VULKAN_H_
#define FE_NK_GLFW_VULKAN_H_
...
nk_glfw3_vk_init(GLFWwindow* win, VkInstance instance, VkDevice logical_device,
...
NK_API void nk_glfw3_vk_shutdown(void);
...
NK_API void nk_gflw3_vk_scroll_callback(GLFWwindow* win, double xoff, double yoff);
NK_API void nk_glfw3_vk_mouse_button_callback(GLFWwindow* win, int button,
    int action, int mods);
...
nk_glfw3_create_and_update_uniform_descriptor_set(struct nk_glfw_device* dev) {
    ...
    allocate_info.pool = dev->descriptor_pool;
...
nk_glfw3_create_texture_descriptor_sets(struct nk_glfw_device* dev) {
    ...
    if (descriptor_set_layouts == NULL || descriptor_sets == NULL)
    {
        FE_CORE_LOG_ERROR("failed to alloc memory for descriptor set layouts and descriptor sets");
        return;
    }
    ...
    if (dev->texture_descriptor_sets == NULL)
    {
        FE_CORE_LOG_ERROR("failed to alloc memory for descriptor set layouts and descriptor sets");
        return;
    }
...
NK_INTERN void nk_glfw3_create_pipeline_layout(struct nk_glfw_device* dev) {
    ...
    VkDescriptorSetLayout descriptor_set_layouts[2] = { 0 };
    VkResult result;
...
NK_INTERN void nk_glfw3_create_pipeline(struct nk_glfw_device* dev) {
    ...
    rasterization_state.depthBiasConstantFactor = 0.0f; // Optionnal
    rasterization_state.depthBiasClamp = 0.0f; // Optionnal
    rasterization_state.depthBiasSlopeFactor = 0.0f; // Optionnal
    ...
    VkPipelineRenderingCreateInfo pipelineRenderingCreateInfo = {
        ...
        .pColorAttachmentFormats = &dev->color_format,
...
NK_INTERN void nk_glfw3_device_upload_atlas(VkQueue graphics_queue,
    ...
    */
    ...
    image_shader_memory_barrier.newLayout = //VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
...
NK_API
void nk_glfw3_vk_shutdown(void) {
    nk_font_atlas_clear(&glfw.atlas);
    nk_free(&glfw.ctx);
    nk_glfw3_vk_device_destroy();
...
NK_API void nk_glfw3_vk_new_frame(void) {
    int i;
    double x, y;
...
NK_API void nk_gflw3_vk_scroll_callback(GLFWwindow* win, double xoff,
    double yoff) {
    (void)win;
    (void)xoff;
...
NK_API void nk_glfw3_vk_mouse_button_callback(GLFWwindow* window, int button,
    int action, int mods) {
    double x, y;
    NK_UNUSED(mods);
...
nk_glfw3_vk_init(GLFWwindow* win, VkInstance instance, VkDevice logical_device,
    ...
    if (init_state == NK_GLFW3_INSTALL_CALLBACKS) {
        glfwSetScrollCallback(win, nk_gflw3_vk_scroll_callback);
        glfwSetCharCallback(win, nk_glfw3_vk_char_callback);
        glfwSetMouseButtonCallback(win, nk_glfw3_vk_mouse_button_callback);
    }
    ...
    nk_glfw3_vk_device_create(instance, logical_device, physical_device,
